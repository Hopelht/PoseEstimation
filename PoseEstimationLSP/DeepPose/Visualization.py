...
def chart(lossList, accuracyList):
    ...
    ax1.plot(range(0, len(lossList)), lossList, label='Test Loss', color='r')
    ax1.legend(loc='upper right', borderpad=0.4)

    ax2.set(xlabel='Epoch', ylabel='Accuracy')
    ax2.plot(range(0, len(accuracyList)), accuracyList, label='Test Accuracy')
...
def print_sample(predict, labels, orim):
    ...
    for idx in range(14):
        ax1.plot(labels[idx][0], labels[idx][1], '.', color='r')

    ax2.title.set_text('predict joints')
    ax2.imshow(normalized_img)
    for idx in range(14):
        ax2.plot(predict[idx][0], predict[idx][1], '.', color='r')

#运行主入口
from dataset_hourglass import PoseImageDataset
...
#模型
model = HourglassNet(Bottleneck, num_stacks=2, num_blocks=4, num_classes=2).to(DEVICE)
...
lossFunction = torch.nn.MSELoss(reduction='sum')  #损失函数
...
for idx, _ in enumerate(predict_):
    print(predict_[idx][0].detach().numpy())
...
Visualization.print_sample(predicts, labels_, orim)  #原始joints和预测joints可视化
